    ...
    async def _ingest():
        ...
        async with pool.acquire() as conn:
            ...
            if target in ("crosswalk_soc_naics", "all"):
                from world_of_taxanomy.ingest.crosswalk_soc_naics import ingest_crosswalk_soc_naics
            ...
            if target in ("crosswalk_isco_isic", "all"):
                from world_of_taxanomy.ingest.crosswalk_isco_isic import ingest_crosswalk_isco_isic
            ...
            if target in ("crosswalk_isced_isco", "all"):
                from world_of_taxanomy.ingest.crosswalk_isced_isco import ingest_crosswalk_isced_isco
            ...
            if target in ("domain_truck_pricing", "all"):
                from world_of_taxanomy.ingest.domain_truck_pricing import ingest_domain_truck_pricing
            ...
            if target in ("domain_truck_regulatory", "all"):
                from world_of_taxanomy.ingest.domain_truck_regulatory import ingest_domain_truck_regulatory
            ...
            if target in ("domain_truck_tech", "all"):
                from world_of_taxanomy.ingest.domain_truck_tech import ingest_domain_truck_tech
            ...
            if target in ("domain_truck_lane", "all"):
                from world_of_taxanomy.ingest.domain_truck_lane import ingest_domain_truck_lane
            ...
            if target in ("domain_ag_equipment", "all"):
                from world_of_taxanomy.ingest.domain_ag_equipment import ingest_domain_ag_equipment
            ...
            if target in ("domain_ag_input", "all"):
                from world_of_taxanomy.ingest.domain_ag_input import ingest_domain_ag_input
            ...
            if target in ("domain_ag_business", "all"):
                from world_of_taxanomy.ingest.domain_ag_business import ingest_domain_ag_business
            ...
            if target in ("domain_ag_market", "all"):
                from world_of_taxanomy.ingest.domain_ag_market import ingest_domain_ag_market
            ...
            if target in ("domain_ag_regulatory", "all"):
                from world_of_taxanomy.ingest.domain_ag_regulatory import ingest_domain_ag_regulatory
            ...
            if target in ("domain_ag_land", "all"):
                from world_of_taxanomy.ingest.domain_ag_land import ingest_domain_ag_land
            ...
            if target in ("domain_ag_postharvest", "all"):
                from world_of_taxanomy.ingest.domain_ag_postharvest import ingest_domain_ag_postharvest
            ...
            if target in ("domain_energy_storage", "all"):
                from world_of_taxanomy.ingest.domain_energy_storage import ingest_domain_energy_storage
                print("\n-- Domain: New Energy Storage Types (hand-coded) --")
                n = await ingest_domain_energy_storage(conn)
                print(f"  {n} nodes")

            if target in ("domain_semiconductor", "all"):
                from world_of_taxanomy.ingest.domain_semiconductor import ingest_domain_semiconductor
            ...
            if target in ("domain_synbio", "all"):
                from world_of_taxanomy.ingest.domain_synbio import ingest_domain_synbio
                print("\n-- Domain: Synthetic Biology Types (hand-coded) --")
                n = await ingest_domain_synbio(conn)
                print(f"  {n} nodes")

            if target in ("domain_xr_meta", "all"):
                from world_of_taxanomy.ingest.domain_xr_meta import ingest_domain_xr_meta
                print("\n-- Domain: Extended Reality and Metaverse Types (hand-coded) --")
                n = await ingest_domain_xr_meta(conn)
                print(f"  {n} nodes")

            if target in ("cnae_2012", "all"):
                from world_of_taxanomy.ingest.cnae_2012 import ingest_cnae_2012
                print("\n-- CNAE 2.0 (Brazil) --")
                n = await ingest_cnae_2012(conn)
                print(f"  {n} nodes")

            if target in ("csic_2017", "all"):
                from world_of_taxanomy.ingest.csic_2017 import ingest_csic_2017
                print("\n-- CSIC 2017 (China) --")
                n = await ingest_csic_2017(conn)
                print(f"  {n} nodes")

            if target in ("okved_2", "all"):
                from world_of_taxanomy.ingest.okved_2 import ingest_okved_2
                print("\n-- OKVED-2 (Russia) --")
                n = await ingest_okved_2(conn)
                print(f"  {n} nodes")

            if target in ("kbli_2020", "all"):
                from world_of_taxanomy.ingest.kbli_2020 import ingest_kbli_2020
                print("\n-- KBLI 2020 (Indonesia) --")
                n = await ingest_kbli_2020(conn)
                print(f"  {n} nodes")

            if target in ("scian_2018", "all"):
                from world_of_taxanomy.ingest.scian_2018 import ingest_scian_2018
                print("\n-- SCIAN 2018 (Mexico) --")
                n = await ingest_scian_2018(conn)
                print(f"  {n} nodes")

            if target in ("sic_sa", "all"):
                from world_of_taxanomy.ingest.sic_sa import ingest_sic_sa
                print("\n-- SIC-SA (South Africa) --")
                n = await ingest_sic_sa(conn)
                print(f"  {n} nodes")

            if target in ("crosswalk_geo_sector", "all"):
                from world_of_taxanomy.ingest.crosswalk_geo_sector import ingest_crosswalk_geo_sector
            ...
            if target in ("crosswalk_country_system", "all"):
                from world_of_taxanomy.ingest.crosswalk_country_system import ingest_crosswalk_country_system
