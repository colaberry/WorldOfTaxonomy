...
def main() -> int:
    ...
    parser.add_argument(
        "--min-overlap", type=float, default=_DEFAULT_MIN_OVERLAP,
        help=(
            "Minimum target/source code overlap (0..1) before mirroring. "
            f"Default {_DEFAULT_MIN_OVERLAP}. Lower this when mirroring "
            "from a small canonical (e.g. schedule_b 119 codes) into a "
            "much larger target (hs_2022 6,961 codes) where only the "
            "shared chapter codes are expected to align."
        ),
    )
