"""Backfill WHO ATC node descriptions from the atc_who.csv on disk.

Surfaces the ``ddd``, ``uom``, ``adm_r`` (administration route), and
``note`` columns into ``classification_node.description`` for rows
where any of those columns carries non-NA content.
"""
...
async def _run(dry_run: bool) -> int:
    ...
    mapping = parse_atc_who_descriptions(source)
