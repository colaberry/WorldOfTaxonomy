...
| `types.ts` | Mirrors backend Pydantic models: `ClassificationSystem`, `SystemDetail`, `ClassificationNode`, `Equivalence`, `CrosswalkStat`, `CrosswalkGraphResponse`, `User`, `ApiKey`, `AuthTokens`, etc. Keep in sync with `world_of_taxonomy/api/schemas.py` |
