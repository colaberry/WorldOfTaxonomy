...
def cmd_ingest(args):
    ...
    async def _ingest():
        ...
        async with pool.acquire() as conn:
            ...
            if target in ("gs1_gpc", "all"):
                from world_of_taxonomy.ingest.gs1_gpc import ingest_gs1_gpc
                print("\n-- GS1 GPC (Global Product Classification, Nov 2025) --")
                n = await ingest_gs1_gpc(conn)
                print(f"  {n} nodes")

            if target in ("apqc_pcf", "all"):
                from world_of_taxonomy.ingest.apqc_pcf import ingest_apqc_pcf
                print("\n-- APQC PCF (Cross-Industry, Level-1 skeleton) --")
                n = await ingest_apqc_pcf(conn)
                print(f"  {n} nodes")

            if target in ("crosswalk_apqc_anchors", "all"):
                from world_of_taxonomy.ingest.crosswalk_apqc_anchors import (
                    ingest_crosswalk_apqc_anchors,
                )
                print("\n-- APQC PCF anchor crosswalks (SCOR / ITIL / COBIT / PMBOK) --")
                n = await ingest_crosswalk_apqc_anchors(conn)
                print(f"  {n} edges")
