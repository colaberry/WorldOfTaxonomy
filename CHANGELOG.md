...
- **GS1 GPC** (`gs1_gpc`): 6,450 nodes covering the November 2025 GS1 Global Product Classification (45 segments, 162 families, 937 classes, 5,306 bricks). Sourced from the GS1 GPC Browser 'As Published' GB locale XLSX. Hierarchical 4-level tree using authoritative 8-digit codes throughout. 100% description coverage (Brick definitions are always populated by GS1; Segment/Family/Class fall back to title when the source definition is empty). Attributes (8-digit codes starting with 2) are GPC brick-level qualifiers, not hierarchy, and are not ingested as nodes. Closes the GS1 GPC gap from the WoO inclusion-policy audit and completes Tier 1 + Tier 2 ingestion.
